...
namespace
{
    constexpr const wchar_t* kWindowClassName = L"MiniD3D11RendererWindowClass";
}
...
bool Window::Create(HINSTANCE hInstance, const wchar_t* title, UINT width, UINT height)
{
    ...
    {
        MessageBoxW(nullptr, L"윈도우 클래스 등록에 실패했습니다.", L"MiniD3D11Renderer", MB_OK | MB_ICONERROR);
        return false;
    }
    ...
    if (!m_hwnd)
    {
        MessageBoxW(nullptr, L"창 생성에 실패했습니다.", L"MiniD3D11Renderer", MB_OK | MB_ICONERROR);
        return false;
    }
